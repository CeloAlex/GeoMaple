...
type NoLote = { lote: string; imoveis: ImovelRegistro[] }
type NoQuadra = { qu: string; lotes: NoLote[] }
...
function montarArvore(imoveis: ImovelRegistro[]): NoDistrito[] {
  const distritos = new Map<string, Map<string, Map<string, Map<string, ImovelRegistro[]>>>>()

  for (const im of imoveis) {
    const partes = parseInscricao(im.insc)
    if (!partes) continue
    const { di, se, qu, lote } = partes
    ...
    if (!quadras.has(qu)) quadras.set(qu, new Map())
    const lotes = quadras.get(qu)!
    if (!lotes.has(lote)) lotes.set(lote, [])
    lotes.get(lote)!.push(im)
  }
        ...
        .map(([se, quadras]) => ({
          ...
          quadras: [...quadras.entries()]
            ...
            .map(([qu, lotes]) => ({
              ...
              lotes: [...lotes.entries()]
                .sort(([a], [b]) => a.localeCompare(b))
                .map(([lote, imoveisList]) => ({
                  lote,
                  imoveis: imoveisList.sort((a, b) => a.insc.localeCompare(b.insc)),
                })),
            })),
        })),
    }))
                      ...
                      return (
                        <details key={q.qu}>
                          <summary className="flex cursor-pointer select-none items-center rounded px-1.5 py-1 text-white/70 hover:bg-white/10">
                            ...
                            Quadra {q.qu} ({q.lotes.reduce((n, l) => n + l.imoveis.length, 0)})
                          </summary>
                          <div className="ml-3 space-y-1 border-l border-white/10 pl-2">
                            {q.lotes.map((l) => {
                              const chaveLote = `${chaveQuadra}.${l.lote}`
                              return (
                                <details key={l.lote} open>
                                  <summary className="flex cursor-pointer select-none items-center rounded px-1.5 py-1 text-white/60 hover:bg-white/10">
                                    <Visibilidade chave={chaveLote} ramosOcultos={ramosOcultos} onAlternarRamo={onAlternarRamo} />
                                    Lote {l.lote}
                                  </summary>
                                  <div className="ml-3 space-y-0.5 border-l border-white/10 pl-2">
                                    {l.imoveis.map((im) => (
                                      ...
                                      >
                                        Unidade {im.insc.split('-')[1]} – {im.prop}
                                      </button>
                                    ))}
                                  </div>
                                </details>
